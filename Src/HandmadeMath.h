...
static inline float DeterminantM3(Mat3 Matrix)
{
    ...
    Mat3 _Cross;
    _Cross.Columns[0] = Cross(Matrix.Columns[1], Matrix.Columns[2]);
    _Cross.Columns[1] = Cross(Matrix.Columns[2], Matrix.Columns[0]);
    _Cross.Columns[2] = Cross(Matrix.Columns[0], Matrix.Columns[1]);

    return DotV3(_Cross.Columns[2], Matrix.Columns[2]);
}
...
static inline Mat3 InvGeneralM3(Mat3 Matrix)
{
    ...
    float InvDeterminant = 1.0f / DotV3(_Cross.Columns[2], Matrix.Columns[2]);

    Mat3 Result;
    Result.Columns[0] = MulV3F(_Cross.Columns[0], InvDeterminant);
    Result.Columns[1] = MulV3F(_Cross.Columns[1], InvDeterminant);
    Result.Columns[2] = MulV3F(_Cross.Columns[2], InvDeterminant);
...
COVERAGE(NormQ, 1)
static inline Quat NormQ(Quat _Quat)
{
    ASSERT_COVERED(NormQ);

    /* NOTE(lcf): Take advantage of SSE implementation in NormV4 */
    Vec4 Vec = {_Quat.X, _Quat.Y, _Quat.Z, _Quat.W};
    Vec = NormV4(Vec);
